...
pf.analyze(tolerance=0.2, action_tolerance=0.1, orientation='u', mlc_model='MLCi2')
pf.plot_analyzed_image()
print(pf.results())
pf.publish_pdf(filename='./Arquivos_de_teste/teste_elekta2.pdf', customized=True)


pickets = pf.get_test_pickets()
errors = np.zeros([pickets[0].error_array.shape[0],len(pickets)+1])
text_file = open('./Arquivos_de_teste/teste_elekta2.txt', "w")
