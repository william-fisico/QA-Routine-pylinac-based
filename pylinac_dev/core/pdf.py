...
class PylinacCanvas:
    ...
    def __init__(self, filename: str, page_title: str, font: str="Helvetica", metadata: dict=None, metadata_location: tuple=(2, 25.5), customized: bool=False, image_reader=None):
    ...
    def _generate_pylinac_template_theme(self):
        ...
        self.canvas.drawImage(osp.join(osp.dirname(osp.dirname(osp.abspath(__file__))), 'files', 'PylinacFullcropped.png'),
    ...
    def _generate_pylinac_template_theme_customized(self,image_reader):
        # draw logo and header separation line
        self.canvas.drawImage(image_reader,1 * cm, 26.5 * cm, width=5 * cm, height=3 * cm, preserveAspectRatio=True)
        self.canvas.line(1 * cm, 26.5 * cm, 20 * cm, 26.5 * cm)
        # draw title
        self.add_text(text=self._title, location=(7, 28), font_size=24)
        # draw "generated by pylinac" tag
