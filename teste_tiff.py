...
def teste_tiff(nome):
    #Carregando arquivo TIFF
    tiff_file = Image.open(nome)
    tiff_array = np.array(tiff_file)
    tiff_meta_dict = {TAGS[key] : tiff_file.tag[key] for key in tiff_file.tag.keys()}
    print(nome)
    print(tiff_meta_dict)
    x = 1.6/(tiff_meta_dict['YResolution'][0][0]/(tiff_meta_dict['YResolution'][0][1]*25.4))
    print(x)
    print('-------------------------')


nomes = ['./6mm-5MU.tif','./PELICULA1_bg_001.tif']
for nome in nomes:
    teste_tiff(nome)